...
Offline regeneration proposals and structured Prompt Contract previews are available at `/projects/:projectId/visual-regeneration`. The companion `scripts/plan-visual-regeneration.ps1 -PlanOnly` command is read-only by default and never authorizes paid execution. See [the regeneration policy](docs/phase-3c-visual-regeneration-policy.md).
...
TOAPIS two-Shot verification is persisted as a resumable `ProviderVerificationRun`. The default `scripts/e2e-real-provider.ps1` command is offline and cannot incur cost; `-ConfirmLive` alone also cannot incur cost. A live run additionally requires `-ExecutePaid`, a reviewed pricing hash, an explicit `TOAPIS_CREDIT` ceiling, and the other backend gates. Re-running with `-RunId` resumes the database state rather than resubmitting completed work. `-AutoApproveForVerification` means workflow-verification approval only, never human visual or production-quality approval. Shot 2 continuity comes exclusively from Shot 1's locally extracted and validated FFmpeg tail; Provider `last_frame_url` metadata is not a continuity input. The script attempts to disable live orchestration in `finally` on success, failure, timeout, or interruption.
