...
const workspaceContext = inject(projectWorkspaceKey, null);
...
const projectId = computed(() =>
  Number(route.params.projectId ?? route.params.id),
);
const selected = computed(() => store.selectedShot);
const videos = computed(() =>
  selected.value ? store.assetsByShot(selected.value.id, "VIDEO") : [],
);
...
const selectableProviders = computed(() =>
  providerProfiles.value.filter((p) => p.enabled && !p.archived_at),
);
const imageModels = computed(() =>
  (providerModels.value[settingsForm.value.image_provider_id] ?? []).filter(
    (m) => m.generation_type === "IMAGE",
  ),
);
const videoModels = computed(() =>
  (providerModels.value[settingsForm.value.video_provider_id] ?? []).filter(
    (m) => m.generation_type === "VIDEO",
  ),
);
...
const selectedTaskGroups = computed(() => {
  const requests = new Map(
    (store.current?.requests ?? []).map((request) => [request.id, request]),
  );
...
const latestRender = computed(
  () => [...(store.current?.renders ?? [])].reverse()[0] ?? null,
);
const generationWorkerOnline = computed(
  () => (store.workerStatus?.generation.online_count ?? 0) > 0,
);
const resultWorkerOnline = computed(
  () => (store.workerStatus?.result.online_count ?? 0) > 0,
);
const renderWorkerOnline = computed(
  () => (store.workerStatus?.render.online_count ?? 0) > 0,
);
const completedShotCount = computed(
  () => store.current?.completion.completed_shots ?? 0,
);
const totalShotCount = computed(
  () => store.current?.completion.total_shots ?? 0,
);
const settingsSummary = computed(() => {
  const imageProvider = selectableProviders.value.find(
    (item) => item.provider_key === settingsForm.value.image_provider_id,
  );
  const videoProvider = selectableProviders.value.find(
    (item) => item.provider_key === settingsForm.value.video_provider_id,
  );
  const imageModel = imageModels.value.find(
    (item) => item.model_key === settingsForm.value.image_model,
  );
  const videoModel = videoModels.value.find(
    (item) => item.model_key === settingsForm.value.video_model,
  );
...
onMounted(async () => {
  if (workspaceContext) await workspaceContext.ready.value;
  else await store.loadProject(projectId.value);
  syncSettingsForm();
  ...
  if (typeof api.listProviderProfiles === "function") {
    ...
    await Promise.all(
      providerProfiles.value.map(async (p) => {
        providerModels.value[p.provider_key] = await api.listProviderModels(
          p.id,
        );
      }),
    );
...
watch(
  ...
  (shot) => {
    if (!shot) return;
    activeMediaStep.value =
      shot.status === "KEYFRAME_REVIEW"
        ? "keyframe"
        : shot.status === "VIDEO_REVIEW"
          ? "video"
          : shot.status === "COMPLETED" || shot.status === "TAIL_FRAME_LOCKED"
            ? "tail"
            : "start";
  },
  { immediate: true },
  ...
  try {
    await store.updateProjectSettings({
      ...
      default_video_duration_seconds:
        settingsForm.value.default_video_duration_seconds,
    ...
    ElMessage.error(
      error instanceof Error ? error.message : "保存生成设置失败",
    );
  } finally {
...
function generationOptions(kind: "keyframe" | "video"): GenerationStartOptions {
  return {
    provider_id:
      kind === "keyframe"
        ? settingsForm.value.image_provider_id || null
        : settingsForm.value.video_provider_id || null,
    model:
      kind === "keyframe"
        ? settingsForm.value.image_model || null
        : settingsForm.value.video_model || null,
...
function sourceLabel(asset: ShotAssetSummary | null) {
  ...
  if (asset.source_type === "inherited") {
    const source =
      asset.source_shot_title ?? `Shot ${asset.source_shot_id ?? ""}`;
    return `继承自 ${source}`;
  }
...
function imageProviderChanged() {
  if (
    !imageModels.value.some(
      (m) => m.model_key === settingsForm.value.image_model,
    )
  )
    settingsForm.value.image_model = "";
}
function videoProviderChanged() {
  if (
    !videoModels.value.some(
      (m) => m.model_key === settingsForm.value.video_model,
    )
  )
    settingsForm.value.video_model = "";
}
...
function qualityLine(item: QualityCheckResult) {
  ...
  const threshold =
    item.threshold === null ? "" : `阈值 ${formatMetric(item.threshold)}`;
...
function displayShotTitle(shot: Shot) {
  return /^Shot\s+\d+$/i.test(shot.title)
    ? `镜头 ${shot.sort_order + 1}`
    : shot.title;
}
...
async function archiveCurrentProject() {
  ...
  try {
    await ElMessageBox.confirm(
      `移除项目“${store.current.name}”？之后可在已移除项目中恢复。`,
      "移除项目",
...
function canGenerateVideo(shot: Shot) {
  return (
    shot.actions?.can_generate_video ?? shot.status === "KEYFRAME_APPROVED"
  );
}
...
function taskStage(task: GenerationTask) {
  ...
  if (task.status === "RUNNING")
    return task.processing_stage
      ? statusLabel(task.processing_stage)
      : "远端生成中";
...
async function deleteShot(shot: Shot, event: MouseEvent) {
  ...
  try {
    await ElMessageBox.confirm(
      `删除“${shot.title || `镜头 ${shot.sort_order + 1}`}”后将无法恢复，是否继续？`,
      "确认删除镜头",
      {
...
async function dropOn(target: Shot) {
  ...
  draggedShotId.value = null;
  await guarded(() =>
    store.reorder(store.current?.id ?? projectId.value, shots),
  );
}
...
function taskType(task: GenerationTask) {
  ...
  if (task.status === "RESULT_READY" || task.status === "PROCESSING_RESULT")
    return "primary";
  if (task.status === "RETRY_WAIT" || task.status === "CANCELLING")
    return "warning";
  return "primary";
}
...
async function cancelTask(task: GenerationTask) {
  ...
  try {
    await api.cancelTask(
      task.id,
      "Cancelled from project task panel.",
...
async function retryTask(task: GenerationTask) {
  ...
  try {
    await api.retryTask(
      task.id,
      "Manual retry from project task panel.",
      idempotencyKey("retry", task.id),
    );
    await store.refreshProjectDetail();
    startPolling();
...
async function createRender() {
  ...
  try {
    await api.createProjectRender(
      store.current.id,
      idempotencyKey("render", store.current.id),
    );
    await store.refreshProjectDetail();
    startPolling();
...
async function uploadAndSetStartFrame(event: Event) {
  ...
  await guarded(async () => {
    const asset = await api.uploadProjectImage(
      store.current?.id ?? projectId.value,
      file,
    );
    await api.setStartFrame(selected.value?.id ?? 0, {
      action: "SELECT",
      asset_id: asset.id,
    });
  });
...
async function uploadTargetKeyframe(event: Event) {
  ...
  await guarded(async () => {
    ...
    await api.setTargetKeyframe(selected.value?.id ?? 0, {
      asset_id: asset.id,
    });
  });
  (event.target as HTMLInputElement).value = "";
...
<template>
  <main
    ...
    data-testid="workspace-page-workbench"
    v-loading="store.loading"
  >
    <Teleport
      key="workbench-actions"
      to="#project-workspace-actions"
      :disabled="!workspaceContext"
    >
      <el-button
        native-type="button"
        :icon="Refresh"
      ...
      <div>
        <p>
          {{
            ...
            "包含审核、工作进程、服务商与最终渲染的连续性生成工作流。"
          }}
        </p>
        <div class="project-meta">
          <el-tag type="primary">{{
            completedShotCount === totalShotCount && totalShotCount
              ? "已完成"
              : "制作中"
          }}</el-tag
          ><span>{{ totalShotCount }} 个镜头</span
      ...
      <div class="settings-panel">
        ...
        <div class="settings-grid">
          <label>
            图片服务商
            <el-select
              v-model="settingsForm.image_provider_id"
              placeholder="选择图片服务商"
              clearable
              @change="imageProviderChanged"
            >
              <el-option
          ...
          <label>
            视频服务商
            <el-select
              v-model="settingsForm.video_provider_id"
              placeholder="选择视频服务商"
              clearable
              @change="videoProviderChanged"
            >
              <el-option
                v-for="provider in selectableProviders"
                :key="provider.id"
          ...
          <label>
            图片模型
            <el-select
              v-model="settingsForm.image_model"
              :disabled="!settingsForm.image_provider_id"
              placeholder="选择图片模型"
              ><el-option
                v-for="model in imageModels"
                :key="model.id"
                :label="`${model.display_name} · ${model.remote_model || model.model_key}${model.enabled ? '' : ' · 已停用'}`"
          ...
          <label>
            视频模型
            <el-select
              v-model="settingsForm.video_model"
              :disabled="!settingsForm.video_provider_id"
              placeholder="选择视频模型"
              ><el-option
          ...
          <label>
            时长（秒）
            <el-input-number
              v-model="settingsForm.default_video_duration_seconds"
              :min="0.1"
              :max="60"
            />
          </label>
      ...
      <div class="worker-panel">
        <div class="panel-title">
          ...
          <el-button
            native-type="button"
            :icon="Refresh"
            :loading="store.workersRefreshing"
            @click="store.refreshWorkers"
          >
            刷新工作状态
          </el-button>
        </div>
        <div class="worker-row">
          <el-tag :type="generationWorkerOnline ? 'success' : 'danger'">
            生成工作进程
            {{ store.workerStatus?.generation.online_count ?? 0 }}/{{
              store.workerStatus?.generation.total_count ?? 0
            }}
          </el-tag>
          <span v-if="!generationWorkerOnline"
            >启动命令：python -m app.workers.cli generation</span
          >
        </div>
        <div class="worker-row">
          <el-tag :type="renderWorkerOnline ? 'success' : 'danger'"
            >渲染工作进程：{{ renderWorkerOnline ? "在线" : "离线" }}</el-tag
          >
        </div>
        <div class="progress-summary">
          <span
            >当前镜头：{{
              selected ? statusLabel(selected.status) : "暂无"
            }}</span
          ><span>运行任务：{{ store.hasActiveTasks ? "有" : "无" }}</span
          ><span>镜头进度：{{ completedShotCount }} / {{ totalShotCount }}</span
          ><el-progress
            :percentage="
              totalShotCount
                ? Math.round((completedShotCount / totalShotCount) * 100)
                : 0
            "
          />
        </div>
        <div class="worker-row">
          <el-tag :type="resultWorkerOnline ? 'success' : 'danger'">
            结果工作进程 {{ store.workerStatus?.result.online_count ?? 0 }}/{{
              store.workerStatus?.result.total_count ?? 0
            }}
          </el-tag>
          <span v-if="!resultWorkerOnline"
            >启动命令：python -m app.workers.result_cli run</span
          >
        </div>
      </div>
    </section>
    ...
    <section class="layout">
      <aside class="timeline">
        ...
        >
          ...
          <el-tag size="small" :type="statusType(shot.status)">{{
            statusLabel(shot.status)
          }}</el-tag>
          <div class="shot-flags">
            <span>{{
              shot.approved_keyframe_asset_id ? "关键帧已通过" : "关键帧待处理"
            }}</span
            ><span>{{
              shot.approved_video_asset_id ? "视频已通过" : "视频待处理"
            }}</span
            ><span>{{
              shot.locked_tail_frame_asset_id ? "尾帧已锁定" : "尾帧未锁定"
            }}</span>
          </div>
          <el-dropdown trigger="click" @click.stop
            ><el-button text aria-label="更多镜头操作">更多</el-button
            ><template #dropdown
              ><el-dropdown-menu
                ><el-dropdown-item
                  class="delete-shot"
                  @click="deleteShot(shot, $event)"
                  >删除镜头</el-dropdown-item
                ></el-dropdown-menu
              ></template
            ></el-dropdown
          >
      ...
      <section v-if="selected" class="review-grid">
        <div class="editor-panel">
          ...
          <div class="revision-meta">
            ...
            <el-tag size="small" type="info">{{
              statusLabel(selected.start_frame_source_type)
            }}</el-tag>
            <span
              >关键帧 #{{ selected.approved_keyframe_asset_id ?? "无" }}</span
            >
          ...
          <el-form label-position="top">
            <el-form-item label="标题">
              <el-input
                ...
                @change="store.updateSelectedShot({ title: String($event) })"
              />
            </el-form-item>
            <el-form-item label="描述">
              <el-input
                ...
                :rows="2"
                @change="
                  store.reviseSelectedShot('Description changed', {
            ...
            <el-form-item label="时长（秒）">
              <el-input-number
                ...
                :max="60"
                @change="
                  store.reviseSelectedShot('Duration changed', {
                    duration_seconds: Number($event),
                  })
                "
              />
            </el-form-item>
            <el-form-item label="提示词（Prompt）">
              <el-input
                ...
                :rows="4"
                @change="
                  store.reviseSelectedShot('Prompt changed', {
                    prompt: String($event),
                  })
                "
              />
            ...
            <el-form-item label="负面提示词">
              <el-input
                ...
                :rows="3"
                @change="
                  store.reviseSelectedShot('Negative prompt changed', {
                    negative_prompt: String($event),
                  })
                "
              />
            </el-form-item>
        ...
        <section class="shot-media-workspace">
          <nav class="media-steps" aria-label="镜头素材流程">
            <button
              :class="{ active: activeMediaStep === 'start' }"
              @click="activeMediaStep = 'start'"
            >
              1 起始帧</button
            ><button
              :class="{ active: activeMediaStep === 'keyframe' }"
              @click="activeMediaStep = 'keyframe'"
            >
              2 目标关键帧</button
            ><button
              :class="{ active: activeMediaStep === 'video' }"
              @click="activeMediaStep = 'video'"
            >
              3 视频</button
            ><button
              :class="{ active: activeMediaStep === 'tail' }"
              @click="activeMediaStep = 'tail'"
            >
              4 已锁定尾帧
            </button>
          </nav>
          ...
          <div
            v-show="activeMediaStep === 'start'"
            class="asset-panel media-detail"
          >
            <div class="panel-title">
              <h2>起始帧</h2>
              <div class="button-row compact">
                ...
                <el-button
                  ...
                  @click="
                    guarded(() =>
                      api.setStartFrame(selectedShotId(), { action: 'CLEAR' }),
                    )
                  "
                >
                  清除
                ...
                <el-button
                  ...
                  @click="
                    guarded(() =>
                      api.setStartFrame(selectedShotId(), {
                        action: 'RESTORE_INHERITED',
                      }),
                    )
                  "
                >
          ...
          <div
            v-show="activeMediaStep === 'keyframe'"
            class="review-panel media-detail"
          >
            <div class="panel-title">
              <h2>目标关键帧</h2>
              <input
                ref="targetKeyframeInput"
                class="hidden-input"
                type="file"
                accept="image/png,image/jpeg,image/webp"
                @change="uploadTargetKeyframe"
              />
              <el-button
                native-type="button"
                :icon="Picture"
                @click="targetKeyframeInput?.click()"
              >
                上传
              </el-button>
              <el-button
                ...
                @click="
                  guarded(
                    () =>
                      store.runAction((shotId) =>
                        api.generateKeyframe(
                          shotId,
                          generationOptions('keyframe'),
                        ),
                      ),
                    true,
                  )
          ...
          >
              ...
              <el-button
                ...
                @click="
                  guarded(
                    () =>
                      store.runAction((shotId) =>
                        api.generateVideo(shotId, generationOptions('video')),
                      ),
                    true,
                  )
                "
            ...
            <div class="asset-strip">
              <video
                v-for="asset in videos"
                :key="asset.id"
                controls
                :src="asset.url"
              />
              <div v-if="!videos.length" class="compact-empty">
                <strong>暂无视频</strong
                ><span>生成完成后可在此预览和审核。</span>
              </div>
            </div>
            <div class="quality-panel">
              <div class="quality-header">
                <strong>
                  质量检查：{{ qualitySummary.warnings }} 个警告，{{
                    qualitySummary.errors
                  }}
                  个错误，{{ qualitySummary.infos }} 个正常
                </strong>
                <el-button
                  native-type="button"
                  size="small"
                  ...
                  :disabled="
                    ...
                    (selected.status !== 'VIDEO_REVIEW' &&
                      !selected.approved_video_asset_id)
                  "
                ...
                <el-button
                  v-if="selectedQualityChecks.length"
                  native-type="button"
                  size="small"
                  text
                  @click="qualityDetailsOpen = !qualityDetailsOpen"
                  >{{ qualityDetailsOpen ? "收起详情" : "查看详情" }}</el-button
                >
              </div>
              <div
                v-if="qualityDetailsOpen && selectedQualityChecks.length"
                class="quality-list"
              >
                <div
                  v-for="item in selectedQualityChecks"
                  :key="item.id"
                  class="quality-row"
                >
                  <el-tag
                    size="small"
                    :type="qualityTagType(item.severity)"
                    :title="item.severity"
                    >{{ statusLabel(item.severity) }}</el-tag
                  >
                  <div>
        ...
        <div class="log-panel">
          ...
          <div class="task-list">
            <div
              v-for="{ task, request } in selectedTaskGroups"
              :key="task.id"
              class="task-row"
            >
              <div v-if="request" class="request-main">
                <strong
                  >请求 #{{ request.id }}
                  {{ statusLabel(request.kind) }}</strong
                >
                <span>{{
                  request.effective_provider_id ?? request.provider_name
                }}</span>
                <span v-if="request.model">模型 {{ request.model }}</span>
                <span v-if="request.generation_mode"
                  >生成模式 {{ statusLabel(request.generation_mode) }}</span
                >
              </div>
              <div class="task-main">
                <strong
                  >#{{ task.id }} {{ statusLabel(task.task_type) }}</strong
                >
                <el-tag size="small" :type="taskType(task)">{{
                  task.status
                }}</el-tag>
              </div>
              <div class="task-meta">
                <span>{{ taskStage(task) }}</span>
                第 {{ task.attempt_number }} 次尝试 / 已重试
                {{ task.retry_count }}/{{ task.max_attempts }} 次
                <span v-if="task.remote_status"
                  >远端状态 {{ statusLabel(task.remote_status) }}</span
                >
                <span v-if="task.remote_job_id"
                  >任务标识 {{ shortRemoteJobId(task.remote_job_id) }}</span
                >
                <span v-if="task.remote_progress !== null"
                  >进度 {{ Math.round(task.remote_progress * 100) }}%</span
                >
                <span v-if="task.next_retry_at"
                  >下次重试 {{ formatDateTime(task.next_retry_at) }}</span
                >
                <span v-if="task.next_result_retry_at"
                  >下次结果重试
                  {{ formatDateTime(task.next_result_retry_at) }}</span
                >
                <span v-if="task.next_poll_at"
                  >下次轮询 {{ formatDateTime(task.next_poll_at) }}</span
                >
                <span v-if="task.job_deadline_at"
                  >任务截止 {{ formatDateTime(task.job_deadline_at) }}</span
                >
                <span v-if="task.cancellation_deadline_at"
                  >取消截止
                  {{ formatDateTime(task.cancellation_deadline_at) }}</span
                >
                <span v-if="task.result_count"
                  >结果 {{ task.result_count }}</span
                >
                <span v-if="task.processing_status"
                  >处理状态 {{ statusLabel(task.processing_status) }}</span
                >
                <span v-if="task.result_asset_id"
                  >素材 #{{ task.result_asset_id }}</span
                >
                <span v-if="task.result_hosts.length"
                  >结果主机 {{ task.result_hosts.join(", ") }}</span
                >
              </div>
              <div
                v-if="task.error_code || task.error_message"
                class="task-error"
              >
          ...
          <el-timeline>
            <el-timeline-item
              v-for="log in store.logsForSelected"
              :key="log.id"
            ...
            >
              <button
                type="button"
                class="log-row"
                @click="toggleLog(log.id, $event)"
              >
                <strong>{{ log.level }}</strong>
                <span>{{ log.message }}</span>
              </button>
              <div v-if="expandedLogIds.has(log.id)" class="log-detail">
                request: {{ log.request_id ?? "none" }} 路 shot:
                {{ log.shot_id ?? "none" }}
              </div>
            </el-timeline-item>
          </el-timeline>
        ...
        <div class="render-panel">
          ...
          <div class="render-summary">
            <span
              >已完成 {{ store.current?.completion.completed_shots ?? 0 }}/{{
                store.current?.completion.total_shots ?? 0
              }}
              个镜头</span
            >
            <span
              ...
              {{
                Math.round(
                  store.current?.completion.estimated_duration_seconds ?? 0,
                )
              }}
              秒</span
            >
            <span v-if="store.current?.completion.render_disabled_reason">{{
              renderDisabledReason(
                store.current.completion.render_disabled_reason,
              )
            }}</span>
          </div>
          <div v-if="latestRender" class="render-card">
            ...
            <div class="task-meta">
              ...
              <span v-if="latestRender.error_code"
                >{{ latestRender.error_code }}:
                {{ latestRender.error_message }}</span
              >
            </div>
            <video
              v-if="latestRender.output_url"
              controls
              :src="latestRender.output_url"
            />
            <a
              v-if="latestRender.output_url"
              :href="latestRender.output_url"
              download
              >下载最终视频</a
            >
          </div>
...
.asset-strip video,
.asset-strip :deep(.el-image),
.asset-card :deep(.el-image) {
  width: 100%;
  height: 100%;
  max-height: 100%;
  object-fit: contain;
