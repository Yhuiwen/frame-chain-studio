...
async function verifyContract(profile: ProviderProfile) {
  ...
  try {
    ...
    ElMessage.success(`接口契约检查：${result.status}`);
    await loadProfiles();
  } catch (error) {
    ElMessage.error(error instanceof Error ? error.message : "接口契约检查失败");
  } finally {
    busy.value = false;
  }
}
...
<template>
  <main class="settings-page" v-loading="loading">
    ...
    <section class="layout">
      <aside class="sidebar">
        <el-button type="primary" :icon="Plus" :loading="busy" @click="createProfile">新建服务商</el-button>
        <el-form label-position="top" class="form">
          <el-form-item label="名称"><el-input v-model="profileForm.name" /></el-form-item>
          <el-form-item label="服务商标识"><el-input v-model="profileForm.provider_key" /></el-form-item>
          <el-form-item label="适配器">
            <el-select v-model="profileForm.adapter_type">
              <el-option label="映射式 HTTP" value="MAPPED_ASYNC_HTTP" />
              <el-option label="TOAPIS" value="TOAPIS" />
              <el-option label="Fake" value="FAKE" />
            </el-select>
          </el-form-item>
          <el-form-item label="基础 URL"><el-input v-model="profileForm.base_url" /></el-form-item>
          <el-form-item label="密钥环境变量"><el-input v-model="profileForm.secret_env_var" /></el-form-item>
          <el-form-item label="配置 JSON"><el-input v-model="profileForm.config" type="textarea" :rows="4" /></el-form-item>
          <el-checkbox v-model="profileForm.enabled">启用</el-checkbox>
        </el-form>
      </aside>
      ...
      <section class="content">
        <el-alert
          ...
          type="info"
          :closable="false"
          title="启动 Frame Chain Studio 前请设置 TOAPIS_API_KEY；应用不会存储或显示该密钥。"
          description="TOAPIS · https://toapis.com/v1 · Seedream 5.0（2K）· Vidu Q3 Pro（720p、关闭音频、两个有序锚点）。远端取消和实时验证仍未验证。"
        />
        <el-card v-if="selectedProfile?.adapter_type === 'TOAPIS' && toapisGate" class="models-panel">
          <template #header><strong>TOAPIS 实时编排门禁</strong></template>
          <p>价格：每次图片请求 6.3 积分；每秒视频 20 积分 · {{ toapisGate.billing_unit }} · {{ toapisGate.pricing_version }}</p>
          <p>双镜头预估：2 × 6.3 + 8 × 20 = {{ toapisGate.estimated_two_shot_billing_units }} 积分；建议上限 {{ toapisGate.recommended_test_ceiling }}。</p>
          <p>价格已审核：{{ toapisGate.pricing_reviewed ? "是" : "需要审核" }} · 模型：{{ toapisGate.preflight.image_model_accessible && toapisGate.preflight.video_model_accessible ? "可访问" : "未验证" }} · 余额已确认：{{ toapisGate.account_balance_sufficient ? "是" : "否" }} · 实时编排：{{ toapisGate.live_orchestration_enabled ? "已启用" : "已停用" }}</p>
          <el-alert type="warning" :closable="false" title="TOAPIS 模型积分、令牌剩余额度与美元是不同计量单位；Frame Chain Studio 不会在它们之间换算。" />
          <div class="gate-actions">
            <el-button :loading="busy" @click="runToApisAction('review')">审核价格</el-button>
            <el-button :loading="busy" @click="runToApisAction('preflight')">检查模型访问</el-button>
            <el-button :loading="busy" @click="runToApisAction('balance')">确认账户容量</el-button>
            <el-button type="primary" :loading="busy" @click="runToApisAction('enable')">启用实时编排</el-button>
            <el-button type="danger" :loading="busy" @click="runToApisAction('disable')">关闭实时编排</el-button>
          </div>
        </el-card>
        <el-table :data="profiles" highlight-current-row @current-change="(row: ProviderProfile | null) => row && loadModels(row.id)">
          ...
          <el-table-column label="Status" width="180">
            <template #default="{ row }">
              <el-tag :type="row.contract_verified ? 'success' : 'info'">接口契约 {{ row.contract_verified ? "已通过" : "待检查" }}</el-tag>
            </template>
          </el-table-column>
          <el-table-column label="Actions" width="360" fixed="right">
            <template #default="{ row }">
              <el-button size="small" :icon="Check" :loading="busy" @click="validateProfile(row)">验证</el-button>
              <el-button size="small" :icon="Check" :loading="busy" @click="verifyContract(row)">合同检查</el-button>
              <el-button size="small" :icon="VideoCamera" :loading="busy" @click="verifyLive(row)">实时验证</el-button>
              <el-button size="small" :icon="CircleClose" @click="archiveProfile(row)">归档</el-button>
            </template>
          </el-table-column>
        </el-table>
        ...
        <section v-if="selectedProfile" class="models-panel">
          <header class="panel-header">
            <h2>{{ selectedProfile.display_name || selectedProfile.name }} 模型</h2>
            <el-button type="primary" :icon="Plus" :loading="busy" @click="createModel">添加模型</el-button>
          </header>
          <el-form label-position="top" class="model-form">
            <el-form-item label="模型标识"><el-input v-model="modelForm.model_key" /></el-form-item>
            <el-form-item label="远端模型"><el-input v-model="modelForm.remote_model" /></el-form-item>
          ...
          <el-table :data="models" stripe>
            ...
            <el-table-column prop="remote_model" label="远端模型" min-width="180" />
