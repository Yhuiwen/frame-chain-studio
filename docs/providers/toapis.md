...
The official documentation index currently links to an OpenAPI 3.1 document whose only paths are the sample `/plants` operations. It does not define TOAPIS models, balances, uploads, images, or videos. Contract decisions are therefore cross-checked against the official endpoint guides, and the incomplete OpenAPI remains a paid-readiness blocker. For Vidu Q3, `seed` is a top-level optional integer, but the first paid verification fixes it to null and omits the field. Video requests include `aspect_ratio` only for text-to-video; one- and two-anchor requests omit it. Before upload, the Worker applies EXIF orientation, converts to RGB, performs deterministic contain resize and median-corner padding to 1280×720, strips metadata, registers derived Assets, then validates both ordered anchors with Pillow.

Run `scripts/create-toapis-verification-anchor.py` to create the ignored local 1280×720 verification anchor. Run `scripts/toapis-paid-readiness.ps1` only after tests and always pass an explicit `-Candidate`; it performs read-only model and balance GETs, opens local SQLite in read-only mode, and never enables live orchestration. `SHORT_CONTINUITY_CANARY` is 2 image tasks plus 2 video tasks at 2 seconds each (92.6 TOAPIS_CREDIT estimate, 110 ceiling, one attempt, no retry). `LEGACY_FULL_TWO_SHOT` preserves 2 image tasks plus 8 total video seconds (172.6 estimate, 190 ceiling). `ready=true` is readiness evidence only, not paid authorization or permission to execute. SHORT has no paid command preview until its real execution entry is separately implemented.
...
## Failed-run recovery planning

Vidu first/last anchors are deterministically normalized to 1280×720 RGB PNG files using contain resize and median-corner padding. Originals remain unchanged; each normalized file is registered as a derived `VIDEO_INPUT_FRAME` Asset with `video-input-v1` evidence.

A failed paid `LIVE_CHAIN` run remains terminal and immutable. `-RecoveryPlanOnly -FailedRunId <id>` reads its lineage, normalized assets, billing evidence, task limits, price review, and balance review to produce a deterministic `recoveryPlanHash`. It does not enable live orchestration, create records, upload, submit, poll, or mutate the database. A future recovery requires separate authorization and the exact current plan hash.
